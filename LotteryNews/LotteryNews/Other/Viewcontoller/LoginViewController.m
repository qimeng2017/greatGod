...
- (UIView *)loginView{
   UIView *loginView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, kScreenWidth, SCREEN_HEIGHT)];
    ...
    UITapGestureRecognizer *cancleTap = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(cancleAction:)];
    [cancaleView addGestureRecognizer:cancleTap];
    [loginView addSubview:cancaleView];
    ...
    [cancaleView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(loginView.mas_top).with.offset(0);
        make.left.mas_equalTo(loginView.mas_left).with.offset(0);
   ...
   [cancaleImageView mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    UIButton *termsServiceBtn = [[UIButton alloc]init];
    [termsServiceBtn setTitle:@"彩大师APP 隐私权保护申明" forState:UIControlStateNormal];
    [termsServiceBtn sizeToFit];
    termsServiceBtn.titleLabel.font = [UIFont fontWithName:@"PingFangSC-Regular" size:12*kScreenWidthP];
    termsServiceBtn.backgroundColor = [UIColor clearColor];
    termsServiceBtn.titleLabel.backgroundColor = [UIColor clearColor];
    [termsServiceBtn setTitleColor:RGBA(67, 181, 223, 1) forState:UIControlStateNormal];
    [termsServiceBtn setTitleColor:[UIColor grayColor] forState:UIControlStateSelected];
    [loginView addSubview:termsServiceBtn];
    [termsServiceBtn addTarget:self action:@selector(toTermsService) forControlEvents:UIControlEventTouchUpInside];
    [termsServiceBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.mas_equalTo(loginView.mas_centerX);
        make.bottom.mas_equalTo(loginView.mas_bottom).with.offset(-20*kScreenWidthP);
    }];
    return loginView;
}
...
- (void)loginSucess{
    if (_delegate && [_delegate respondsToSelector:@selector(userLoginSucess)]) {
        [_delegate userLoginSucess];
    }
...
#pragma toTermsService
- (void)toTermsService{
    TermsServiceViewController *serviceVC = [[TermsServiceViewController alloc]init];
